...
Also investigated, separate from the two patches above: on some Apple Silicon Macs, the very first `CorridorKey Inference`/`CorridorKey Video Inference` call in a freshly-started engine process was once observed to return fully-NaN alpha/foreground/rgba (visibly all-black output), regardless of `auto_despeckle`, with every subsequent call in the same process reliable. Follow-up testing -- a synthetic repro against the real checkpoint at several resolutions, plus a fresh end-to-end run in the desktop app at the production default `img_size=2048` on a newly-launched engine -- could not reproduce it, and ruled out the originally-suspected mechanism: production never actually runs the model under `torch.autocast` on fp32 weights (`CorridorKeyEngine` disables mixed precision whenever `model_precision=torch.float16`, which it always is in practice, so the forward pass runs native fp16 end-to-end). Current best guess is a one-off cold macOS/Metal shader-compilation cache artifact on that machine rather than a reliably-reproducing bug -- if you ever hit a fully black result on a first run, re-run the node; it has not recurred in testing since.
